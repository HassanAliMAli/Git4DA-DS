...
export const MODULE_1_LEVELS: LevelDefinition[] = [
  {
    ...
    setup: async (state) => {
      state.fs.writeFile('/README.md', '# DataPulse Operative Log\nStatus: Initializing...');
    },
    goals: [
      {
        id: 'init',
      ...
      {
        id: 'commit',
        description: 'Sign your first entry with Conventional Commits (feat: init).',
        check: (state) => {
          const commits = state.git.getGraph().commits;
          if (commits.length === 0) return false;
          const latestMessage = commits[0].message.toLowerCase();
          return ['feat:', 'fix:', 'docs:', 'chore:'].some(type => latestMessage.startsWith(type));
        }
      }
    ],
    hints: [
  ...
  },
  {
    ...
    goals: [
      ...
      {
        id: 'clean_commit',
        description: 'Version your analysis while keeping the secrets strictly local.',
        check: (state) => {
          const commits = state.git.getGraph().commits;
  ...
  },
  {
    ...
    setup: async (state) => {
      state.git.init();
      state.fs.writeFile('/revenue_model.py', 'MULTIPLIER = 1.0 # Baseline logic');
      await state.git.add('revenue_model.py');
      await state.git.commit('feat: establish baseline math', 'Dr. Hassan');

      state.fs.writeFile('/revenue_model.py', 'MULTIPLIER = 1.4 # Q4 Aggressive Update');
      await state.git.add('revenue_model.py');
      await state.git.commit('feat: update multiplier for growth', 'Dr. Hassan');
    },
    goals: [
      {
        id: 'view_history',
