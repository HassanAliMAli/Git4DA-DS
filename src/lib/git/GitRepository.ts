...
import { GitObject, GitObjectType, GitCommit, GitTreeEntry, ReflogEntry } from './types';
...
export class GitRepository {
  ...
  public async add(path: string): Promise<void> {
    ...
    if (this.isIgnored(path)) throw new Error(`The following paths are ignored: ${path}`);
  ...
  public async commit(message: string, author: string, signature?: string): Promise<string> {
    if (this.index.size === 0) throw new Error('Nothing to commit (use "git add" first)');
    const rootTreeHash = await this.writeTree();
    const parentHash = this.refs.get(this.head) || null;
    const commitData: GitCommit = { tree: rootTreeHash, parent: parentHash, author, message, timestamp: Date.now(), signature };
  ...
  public async revert(target: string, author: string): Promise<string> {
    if (!this.objects.has(target)) throw new Error(`fatal: bad revision '${target}'`);
    ...
    this.index.clear();
    const parentTree = JSON.parse(this.objects.get((JSON.parse(this.objects.get(commitData.parent)!.data) as GitCommit).tree)!.data) as GitTreeEntry[];
  ...
  public async merge(sourceBranch: string, author: string): Promise<{ status: 'ff' | 'merged' | 'conflict', hash?: string }> {
    ...
    for (const targetEntry of targetTree) {
      ...
      if (currentEntry && currentEntry.hash !== targetEntry.hash) conflicts.push(targetEntry.name);
    ...
    return { status: 'merged', hash: await this.commit(`merge: branch '${sourceBranch}' into ${this.head}`, author) };
  }

  private async getTreeEntries(commitHash: string): Promise<GitTreeEntry[]> {
    const commitObj = this.objects.get(commitHash);
    if (!commitObj) return [];
    const treeObj = this.objects.get((JSON.parse(commitObj.data) as GitCommit).tree);
    return treeObj ? JSON.parse(treeObj.data) as GitTreeEntry[] : [];
  }

  private async restoreStateFromCommit(hash: string): Promise<void> {
    const treeObj = this.objects.get((JSON.parse(this.objects.get(hash)!.data) as GitCommit).tree);
    if (!treeObj) return;
  ...
  public getGraph(): { commits: (GitCommit & { hash: string })[], branches: { name: string, hash: string }[] } {
    ...
    return { commits: Array.from(allCommits.values()).sort((a, b) => b.timestamp - a.timestamp), branches };
  }
  ...
  public async fetch(remote: string): Promise<void> {
    const remoteRefs = this.remotes.get(remote);
    if (!remoteRefs) throw new Error(`fatal: '${remote}' not found`);
    for (const [branch, hash] of remoteRefs.entries()) this.addToReflog(`${remote}/${branch}`, null, hash, `fetch: from ${remote}`);
  }
}
